...
; Les résultats apparaissent dans la fenêtre Debug Output.
; =============================================================================
...
Procedure Assert(condition.b, testName.s)
  g_total + 1
  If condition
    g_pass + 1
    Debug "[PASS] " + testName
...
; =============================================================================
; CASCADE SAVE
; Attention : Protected doit être AVANT ForEach, jamais à l'intérieur du bloc
; =============================================================================
...
Procedure SaveSocieteEmployes(*parent, db.i)
  ...
  Protected *emp.Employe    ; <- déclaration AVANT la boucle
...
; =============================================================================
; HELPERS TESTS : chaque test dans sa procédure pour éviter les conflits
; de portée des variables Protected
; =============================================================================
...
Procedure Test_T02_Migrate()
  ...
  EndIf
  Assert(found, "[T02b] Table 'societes' créée en base")
EndProcedure

; --- T03: Idempotence ---
Procedure Test_T03_Idempotence()
  Protected migOk2.b = ORM_Schema::Migrate(ORM::orm_mainDb)
  Assert(migOk2, "[T03] AutoMigrate 2ème appel idempotent (sans erreur)")
EndProcedure

; --- T04: Insert ---
Global *g_soc1.Societe   ; partagé entre tests
...
Procedure Test_T04_Insert()
  ...
  *g_soc1\actif = 1
  ...
  Assert(*g_soc1\id > 0,     "[T04b] id auto-incrémenté (id=" + Str(*g_soc1\id) + ")")
  Assert(*g_soc1\orm_isNew = #False, "[T04c] orm_isNew = #False après Insert")
EndProcedure
...
Procedure Test_T06_Update()
  ...
  *g_soc1\orm_isDirty = #True
  ...
  Assert(*g_soc1\orm_isDirty = #False, "[T06b] orm_isDirty = #False après Update")
  ...
  Assert(*rel\ville = "Marseille", "[T06c] Ville mise à jour en base: '" + *rel\ville + "'")
  FreeStructure(*rel)
EndProcedure

; --- T07: Query ---
Global *g_soc2.Societe   ; société inactive, partagée pour T13
...
Procedure Test_T07_Query()
  ...
  *g_soc2\actif = 0
  ...
  Assert(ListSize(results()) = 1, "[T07b] Query WHERE actif=1 -> 1 résultat (count=" + Str(ListSize(results())) + ")")

  ; Libérer les entités allouées par Query
...
Procedure Test_T08_SaveWithChildren()
  ...
  *g_emp2\actif   = 1
  ...
  Assert(*g_soc3\id > 0, "[T08b] Parent id assigné: " + Str(*g_soc3\id))
  Assert(*g_emp1\id > 0, "[T08c] Enfant 1 id assigné: " + Str(*g_emp1\id))
  Assert(*g_emp2\id > 0, "[T08d] Enfant 2 id assigné: " + Str(*g_emp2\id))
...
Procedure Test_T09_RestrictDelete()
  ; *g_soc3 a des enfants -> doit être bloqué
  ...
  Assert(chkRes = #ORM_Entity::#ORM_Success, "[T09b] Parent toujours présent après RestrictDelete bloqué")
  FreeStructure(*chk)
EndProcedure
...
Procedure Test_T10_CascadeDelete()
  ...
  Assert(sgRes = #ORM_Entity::#ORM_Error_NotFound, "[T10b] Parent supprimé en base")
  FreeStructure(*sGone)
  ...
  Assert(egRes = #ORM_Entity::#ORM_Error_NotFound, "[T10c] Enfant supprimé en cascade")
  FreeStructure(*eGone)
EndProcedure
...
Procedure Test_T11_Lock()
  ...
  Assert(ok2 = #False, "[T11b] Double Lock refusé (autre poste)")
EndProcedure
...
Procedure Test_T12_Release()
  ...
  Assert(ok3, "[T12b] Re-Lock après Release réussi")
...
Procedure Test_T13_DeleteDirect()
  ...
  Assert(goneRes = #ORM_Entity::#ORM_Error_NotFound, "[T13b] Record supprimé introuvable")
  FreeStructure(*gone)
EndProcedure
...
Procedure Test_T15_AlterTable()
  ...
  If DatabaseQuery(ORM::orm_mainDb, "PRAGMA table_info(societes);")
    ...
    FinishDatabaseQuery(ORM::orm_mainDb)
  EndIf
  Assert(found, "[T15b] Colonne 'pays' présente dans PRAGMA table_info(societes)")
EndProcedure
...
Debug ""
Debug "=============================================="
Debug "  RÉSULTATS: " + Str(g_pass) + " / " + Str(g_total) + " passés"
If g_fail > 0
  Debug "  ÉCHECS: " + Str(g_fail)
Else
  Debug "  Tous les tests passés ✓"
EndIf
Debug "=============================================="
