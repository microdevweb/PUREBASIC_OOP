...
CompilerIf #CompileWindows | #CompileLinux | #CompileMac
  ...
  Procedure OOP_GenerateConstructor(IncludeSuper.b)
    If *ActiveSource = 0 Or *ActiveSource\EditorGadget = 0
      ProcedureReturn
    EndIf
    
    Protected totalLines.i = SendEditorMessage(#SCI_GETLINECOUNT, 0, 0)
    Protected curPos.i = SendEditorMessage(#SCI_GETCURRENTPOS, 0, 0)
    Protected curLine.i = SendEditorMessage(#SCI_LINEFROMPOSITION, curPos, 0)
    
    ; 1. Find enclosing class
    Protected classLine.i = -1
    Protected className$ = ""
    Protected parentClassName$ = ""
    Protected classIndent$ = ""
    Protected scanL.i
    Protected lText$, lower$, rest$, pClass.i, pExt.i
    
    For scanL = curLine To 0 Step -1
      lText$ = Trim(GetLine(scanL))
      lower$ = LCase(lText$)
      If (Left(lower$, 6) = "class " Or FindString(lower$, " class ") > 0) And Left(lower$, 8) <> "endclass"
        classLine = scanL
        classIndent$ = GetIndentPrefix(GetLine(scanL))
        pClass = FindString(lower$, "class ")
        rest$ = Trim(Mid(lText$, pClass + 6))
        If Right(rest$, 1) = "{"
          rest$ = Trim(Left(rest$, Len(rest$) - 1))
        EndIf
        pExt = FindString(LCase(rest$), "extends ")
        If pExt > 0
          className$ = Trim(Left(rest$, pExt - 1))
          parentClassName$ = Trim(Mid(rest$, pExt + 8))
        Else
          className$ = rest$
        EndIf
        Break
    ...
    If classLine = -1
      ; No class found above cursor, search downwards
      For scanL = 0 To totalLines - 1
        lText$ = Trim(GetLine(scanL))
        lower$ = LCase(lText$)
        If (Left(lower$, 6) = "class " Or FindString(lower$, " class ") > 0) And Left(lower$, 8) <> "endclass"
          classLine = scanL
    ...
    If className$ = ""
      ChangeStatus("Aucune classe PureBasic OOP trouvée.", 2000)
      ProcedureReturn
    EndIf
    
    ; 2. Collect fields of the class
    NewList FieldNames$()
    NewList FieldTypes$()
    
    Protected depth.i = 0
    For scanL = classLine To totalLines - 1
      lText$ = Trim(GetLine(scanL))
      lower$ = LCase(lText$)
      If CountString(lText$, "{") > 0
        depth + CountString(lText$, "{")
      EndIf
      If CountString(lText$, "}") > 0
        depth - CountString(lText$, "}")
        If depth <= 0 And scanL > classLine
          Break
        EndIf
      EndIf
      If Left(lower$, 8) = "endclass"
        Break
      EndIf
      
      ; Extract fields declared in class
      If scanL > classLine
        Protected isField.b = #False
        Protected fieldStr$ = ""
        If Left(lower$, 6) = "field "
          isField = #True
          fieldStr$ = Mid(lText$, 7)
        ElseIf Left(lower$, 13) = "public field "
          isField = #True
          fieldStr$ = Mid(lText$, 14)
        ElseIf Left(lower$, 16) = "protected field "
          isField = #True
          fieldStr$ = Mid(lText$, 17)
        ElseIf Left(lower$, 14) = "private field "
          isField = #True
          fieldStr$ = Mid(lText$, 15)
        EndIf
        
        If isField
          Protected numParts = CountString(fieldStr$, ",") + 1
          Protected pIdx.i
          For pIdx = 1 To numParts
            Protected singleField$ = Trim(StringField(fieldStr$, pIdx, ","))
            If singleField$ <> ""
              Protected fName$ = "", fType$ = ""
              Protected pDot = FindString(singleField$, ".")
              If pDot > 0
                fName$ = Trim(Left(singleField$, pDot - 1))
                fType$ = Trim(Mid(singleField$, pDot))
              Else
                fName$ = singleField$
                fType$ = ".i"
              EndIf
              ...
              EndIf
              If fName$ <> ""
                AddElement(FieldNames$()) : FieldNames$() = fName$
                AddElement(FieldTypes$()) : FieldTypes$() = fType$
              EndIf
            EndIf
          Next pIdx
        EndIf
    ...
    ; 3. Handle Super Constructor parameters
    Protected superParams$ = ""
    Protected superArgs$ = ""
    
    If IncludeSuper And parentClassName$ <> ""
      Protected pLower$ = LCase(parentClassName$)
      If FindString(pLower$, "gadget") > 0
        superParams$ = "x_p.i, y_p.i, w_p.i = 100, h_p.i = 30, text_p.s = " + Chr(34) + Chr(34) + ", flags_p.i = 0"
        superArgs$ = "x_p, y_p, w_p, h_p, text_p, flags_p"
      Else
        superParams$ = ""
        superArgs$ = ""
      EndIf
    ...
    ; 4. Build method parameters and body
    Protected methodIndent$ = classIndent$ + "  "
    Protected bodyIndent$ = methodIndent$ + "  "
    Protected paramsList$ = ""
    Protected bodyCode$ = ""
    
    If superParams$ <> ""
      paramsList$ = superParams$
    EndIf
    
    ResetList(FieldNames$())
    ResetList(FieldTypes$())
    While NextElement(FieldNames$()) And NextElement(FieldTypes$())
      Protected pParamName$ = FieldNames$() + "_p" + FieldTypes$()
      If paramsList$ <> ""
        paramsList$ + ", " + pParamName$
      Else
        paramsList$ = pParamName$
      EndIf
      bodyCode$ + bodyIndent$ + "This\" + FieldNames$() + " = " + FieldNames$() + "_p" + #NewLine
    Wend
    
    ; 5. Assemble full constructor code
    Protected constr$ = #NewLine
    constr$ + methodIndent$ + "Public Method Init(" + paramsList$ + ") {" + #NewLine
    If IncludeSuper And parentClassName$ <> ""
      constr$ + bodyIndent$ + "Super::Init(" + superArgs$ + ")" + #NewLine
    EndIf
    If bodyCode$ <> ""
      constr$ + bodyCode$
    EndIf
    constr$ + methodIndent$ + "}" + #NewLine
    
    ; 6. Insert into editor
    SendEditorMessage(#SCI_REPLACESEL, 0, ToAscii(constr$))
    
    FreeList(FieldNames$())
    FreeList(FieldTypes$())
    ChangeStatus("Constructeur généré pour la classe " + className$, 2000)
  EndProcedure
